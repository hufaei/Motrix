export default {
  ...
  'download-error-code': '错误码 {{errorCode}}',
  'download-error-unknown-reason': '未能取得具体错误原因',
  'download-error-generic-suggestion': '请打开任务详情查看原始错误，并检查存储路径、磁盘空间和网络后重试。',
  'download-error-original': 'aria2 原始信息：{{errorMessage}}',
  'download-error-file-exists-reason': '目标位置已存在同名文件，但缺少对应的 .aria2 续传控制文件；为防止原文件被清空，下载已取消',
  'download-error-file-exists-suggestion': '请先重命名或移走冲突文件，或者换一个存储路径后重新添加任务；确认不需要原文件时，也可以手动删除后重试。',
